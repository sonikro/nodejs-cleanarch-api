...
- **Node 8 LTS:** `nodejs-cleanarch-api` has a minimum Node version
...
``` bash
git clone https://github.com/EndemolShineGroup/nodejs-cleanarch-api.git
cd nodejs-cleanarch-api
yarn
yarn test
```
...
We want to ensure that `nodejs-cleanarch-api` is secure for everyone. If
